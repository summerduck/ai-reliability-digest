"""End-to-end: fixture feeds through the whole pipeline to rendered HTML and dry run."""
from __future__ import annotations

import time
from datetime import UTC, date, datetime, timedelta
from types import SimpleNamespace
from unittest.mock import patch

from conftest import fake_client, full_ranking

from digest import archive
from digest.archive import save_issue
from digest.main import build_digest_html


def _entry(i):
    e = SimpleNamespace(
        title=f"Real story {i}",
        link=f"https://news.example.com/{i}",
        summary=f"<p>Details of story {i} about evals.</p>",
    )
    # Relative to the real clock: build_digest_html filters against now(),
    # so a pinned date would silently age out of the fetch window.
    e.published_parsed = time.gmtime((datetime.now(UTC) - timedelta(days=1)).timestamp())
    return e


def _feeds_file(tmp_path):
    feeds_yaml = tmp_path / "feeds.yaml"
    feeds_yaml.write_text(
        "feeds:\n"
        "  - name: Feed A\n    url: https://a.example.com/rss\n    category: ai\n"
        "  - name: Feed B\n    url: https://b.example.com/rss\n    category: testing\n"
    )
    return feeds_yaml


def test_pipeline_produces_full_email(sample_digest, tmp_path):
    feeds_yaml = _feeds_file(tmp_path)
    ok = SimpleNamespace(entries=[_entry(i) for i in range(8)], bozo=False)
    broken = SimpleNamespace(entries=[], bozo=True)
    client = fake_client([full_ranking(8, score=9), sample_digest])

    with (
        patch("digest.main.load_feeds", lambda: __import__("digest.config", fromlist=["load_feeds"]).load_feeds(feeds_yaml)),
        patch("digest.fetch.feedparser.parse", side_effect=[ok, broken]),
    ):
        subject, html = build_digest_html(client)

    assert sample_digest.headline in subject
    assert sample_digest.headline in html
    assert "Feed B" in html  # broken feed surfaces in the footer
    assert len(client.messages.parse_calls) == 2  # one ranking batch + one analysis


def test_articles_from_past_issues_are_never_ranked_again(sample_digest, tmp_path, monkeypatch):
    """An undated feed keeps serving the same story; the archive must stop it."""
    feeds_yaml = _feeds_file(tmp_path)
    archive_dir = tmp_path / "archive"
    save_issue(
        '<div><h1>Last week</h1>'
        # Same article, cosmetically reformatted by the feed since it was sent.
        '<a href="https://news.example.com/0/?utm_source=rss">Real story 0</a></div>',
        date(2026, 8, 31),
        archive_dir=archive_dir,
    )
    monkeypatch.setattr(archive, "ARCHIVE_DIR", archive_dir)

    ok = SimpleNamespace(entries=[_entry(i) for i in range(8)], bozo=False)
    broken = SimpleNamespace(entries=[], bozo=True)
    client = fake_client([full_ranking(7, score=9), sample_digest])

    with (
        patch("digest.main.load_feeds", lambda: __import__("digest.config", fromlist=["load_feeds"]).load_feeds(feeds_yaml)),
        patch("digest.fetch.feedparser.parse", side_effect=[ok, broken]),
    ):
        build_digest_html(client)

    prompt = client.messages.parse_calls[0]["messages"][0]["content"]
    assert "Real story 0" not in prompt  # already sent, so never re-ranked
    assert "Real story 7" in prompt  # everything else still goes through
